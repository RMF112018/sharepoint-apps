...
### API access (Graph delegated permissions)
- This solution requests delegated Microsoft Graph scopes for directory operations:
  - `People.Read` (recent/related people)
  - `User.ReadBasic.All` (basic directory user search)
- Approve in SharePoint admin center → API access → Pending requests.
- Keep least-privilege; do not add scopes beyond these without review.
...
- API permission prompts match documented scopes (People.Read, User.ReadBasic.All)
